...
write.table(outliers, file = "outliers_methylation_median.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
p<-ggplot(data, aes(x = "", y = Value)) +
  ...
  # scale_y_continuous(limits = c(95, 100)) +  # Set y-axis range
  labs(
  ...
  theme_minimal()
ggsave(filename = "methylation_median.pdf", plot = p, width = 6, height = 4)
