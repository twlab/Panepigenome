...
# Read data
data <- read.delim("example_input_2.2and2.3-mapQandrq_check.log", header = T, sep = "\t", stringsAsFactors = FALSE)
data<-data[,c(1,6)]
...
write.table(outliers, file = "outliers_mapQ30.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
p <- ggplot(data, aes(x = "", y = Value)) +
  ...
  geom_hline(yintercept = 0.1, linetype = "dashed", color = "blue", size = 0.8) +  # Horizontal line at 128
  scale_color_manual(values = c("Normal" = "black", "Outlier" = "red")) +  # Define colors
  labs(
    title = "Proportion of mapQ<30 per file",
    y = "Proportion",
    x = "",
    color = "Point Type"
  ) +
...
ggsave(filename = "mapQ30.pdf", plot = p, width = 6, height = 4)
