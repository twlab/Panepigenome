...
write.table(outliers, file = "outliers_rq.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
p <- ggplot(data, aes(x = "", y = Value)) +
  ...
  labs(
    title = "Proportion of rq<0.99 per file",
    y = "Proportion",
    x = "",
    color = "Point Type"
  ) +
...
ggsave(filename = "rq.pdf", plot = p, width = 6, height = 4)
