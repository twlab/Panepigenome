...
# Read data
data <- read.delim("example_input_4.4_promoter_methylation_median_check.log", sep = "\t", stringsAsFactors = FALSE)
data<-data[,c(1,4)]
...
write.table(outliers, file = "outliers_promoter_methylation_median.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
p<-ggplot(data, aes(x = "", y = Value)) +
  ...
  labs(
    title = "Median methyaltion levels per file",
    y = "Value",
    x = "",
    color = "Point Type"
  ) +
  theme_minimal()
ggsave(filename = "promoter_methylation_median.pdf", plot = p, width = 6, height = 4)
