...
write.table(outliers, file = "outliers_mean_read_length.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
p<-ggplot(data, aes(x = "", y = Value)) +
  ...
  labs(
    # title = "Sequence",
    y = "Mean read length (kb)",
    x = "",
    color = "Point Type"
  ) +
  theme_minimal()+coord_cartesian(ylim = c(0, 30))
ggsave(filename = "Readlength.pdf", plot = p, width = 4, height = 3)
