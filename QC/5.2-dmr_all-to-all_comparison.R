...
dat2.1 = read.table(file.path(path, "example1_input_5.1and5.2_dmr.log"), header=T)
dat2.2 = read.table(file.path(path, "example2_input_5.1and5.2_dmr.log"), header=T)
dat3.1 = read.table(file.path(path, "example1_input_5.1and5.2_dmr.log"), header=T)
dat3.2 = read.table(file.path(path, "example2_input_5.1and5.2_dmr.log"), header=T)

### Select CpGs with coverage ≥ 10
dat1.1<-dat1.1[dat1.1$total_coverage>9,]
dat1.2<-dat1.2[dat1.2$total_coverage>9,]
dat2.1<-dat2.1[dat2.1$total_coverage>9,]
dat2.2<-dat2.2[dat2.2$total_coverage>9,]
dat3.1<-dat3.1[dat3.1$total_coverage>9,]
dat3.2<-dat3.2[dat3.2$total_coverage>9,]

### Columns were renamed according to the DSS package documentation
names(dat1.1) <- c("chr", "pos", "N", "X")
names(dat1.2) <- c("chr", "pos", "N", "X")
names(dat2.1) <- c("chr", "pos", "N", "X")
names(dat2.2) <- c("chr", "pos", "N", "X")
names(dat3.1) <- c("chr", "pos", "N", "X")
names(dat3.2) <- c("chr", "pos", "N", "X")

### Create an object of BSseq class, which is defined in bsseq Bioconductor package
BSobj = makeBSseqData( list(dat1.1, dat1.2, dat2.1, dat2.2, dat3.1, dat3.2),
                       c("C1","C2","C3","N1", "N2","N3") )
