...
# data$Identity...99.. <- as.numeric(sub("%", "", data$Identity...99..))
...
write.table(outliers, file = "outliers_identity.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
p<-ggplot(data, aes(x = "", y = Value)) +
  ...
  # geom_hline(yintercept = 99, linetype = "dashed", color = "blue", linewidth = 1) + 
  labs(
    # title = "Sequence",
    y = "Identity (%)",
    x = "",
...
ggsave(filename = "Identity.pdf", plot = p, width = 4, height = 3)
