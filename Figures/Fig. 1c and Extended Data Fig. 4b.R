...
p <- ggplot(diffs, aes(x = Continent_Abbr, y = female_minus_male, fill = Continent_Abbr)) +
  ...
  labs(
    x = "Population",
    y = "CpG Difference (Female - Male)"
  )

# Save figure
ggsave("CpG_Difference_Female_Male.pdf", plot = p, width = 7, height = 4)
...
p <- ggplot(df, aes(x = Continent_Full, y = CpG_Count, fill = Continent_Full)) +
  ...
  geom_hline(yintercept = 29.303965, linetype = "dashed", color = "#76B7B2", size = 1.2,alpha = 0.6) +
  geom_hline(yintercept = 33.839747, linetype = "dashed", color = "#B07AA1", size = 1.2,alpha = 0.6) +
  scale_fill_manual(values = continent_colors) +
  scale_x_discrete(labels = function(x) continent_abbr[x]) +
  labs(x = NULL, y = "CpGs per haplotype (millions)") +
  facet_wrap(~Sex) +
